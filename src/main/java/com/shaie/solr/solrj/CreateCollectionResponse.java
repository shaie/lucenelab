...
public class CreateCollectionResponse extends CollectionAdminResponse {

    private final List<String> coreNames;

    public CreateCollectionResponse(org.apache.solr.client.solrj.response.CollectionAdminResponse rawResponse) {
        super(rawResponse);

        if (isSuccess()) {
            coreNames = new ArrayList<>();
            for (final Entry<String, Object> entry : getSuccessEntry(rawResponse)) {
                coreNames.add(getCoreName(entry));
            }
        } else {
            coreNames = null;
        }
