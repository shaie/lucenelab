...
public class AnnotatingTokenStreamExample {
    ...
    public static void main(String[] args) throws Exception {
        final String text = "quick brown fox ate the blue red chicken";
        final Tokenizer tokenizer = new WhitespaceTokenizer();
        tokenizer.setReader(new StringReader(text));
        final TeeSinkTokenFilter teeSink = new TeeSinkTokenFilter(tokenizer);
        final TokenStream colors = new AnnotatingTokenFilter(teeSink.newSinkTokenStream(new ColorsSinkFilter()),
                COLOR_ANNOT_TERM);
        ...
        teeSink.reset();
        final CharTermAttribute termAtt = teeSink.getAttribute(CharTermAttribute.class);
        final PositionIncrementAttribute termPosAtt = teeSink.getAttribute(PositionIncrementAttribute.class);
        int termsPos = -1;
        ...
        final CharTermAttribute colorAtt = colors.getAttribute(CharTermAttribute.class);
        final PayloadAttribute payloadAtt = colors.getAttribute(PayloadAttribute.class);
